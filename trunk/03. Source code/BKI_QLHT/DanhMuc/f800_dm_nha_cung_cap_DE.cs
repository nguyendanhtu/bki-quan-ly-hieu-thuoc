    ...
    public partial class f800_dm_nha_cung_cap_DE : Form
    {
            ...
            form_2_us_obj();
                ...
                switch (m_e_form_mode)
                {
                    case DataEntryFormMode.InsertDataState:
                         DS_DM_NCC_NSX_NHASX v_ds_dm_nha_cc = new DS_DM_NCC_NSX_NHASX();
                    US_DM_NCC_NSX_NHASX v_us_dm_nha_cc = new US_DM_NCC_NSX_NHASX();
                    v_us_dm_nha_cc.FillDataseByMaNCC(v_ds_dm_nha_cc, m_us_dm_ncc.strMA_NCC);
                    if (v_ds_dm_nha_cc.DM_NCC_NSX_NHASX.Count == 0)
                    {
                                m_us_dm_ncc.Insert();
        ...
        private void m_cmd_save_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    BaseMessages.MsgBox_Infor("Bạn phải nhập tên nhà cung cấp");
                }
            }
            catch (Exception v_e)
            {
        ...
        private void m_txt_sdt_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        BaseMessages.MsgBox_Infor("Bạn cần nhập tên nhà cung cấp");
                    }
                }
            }
            catch (Exception v_e)

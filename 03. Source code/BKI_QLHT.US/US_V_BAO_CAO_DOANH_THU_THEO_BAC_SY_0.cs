///************************************************
/// Generated by: TuNA
/// Date: 10-05-2014 06:16:28
/// Goal: Create User Service Class for V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0
///************************************************
/// <summary>
/// Create User Service Class for V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0
/// </summary>

using System;
using BKI_QLHT;
using IP.Core.IPCommon;
using IP.Core.IPUserService;
using System.Data.SqlClient;
using System.Data;
using BKI_QLHT.DS;


namespace BKI_QLHT.US
{

public class US_V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0 : US_Object
{
	private const string c_TableName = "V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0";
#region "Public Properties"
	public decimal dcID_BAC_SY 
	{
		get
		{
			return CNull.RowNVLDecimal(pm_objDR, "ID_BAC_SY", IPConstants.c_DefaultDecimal);
		}
		set	
		{
			pm_objDR["ID_BAC_SY"] = value;
		}
	}

	public bool IsID_BAC_SYNull()	{
		return pm_objDR.IsNull("ID_BAC_SY");
	}

	public void SetID_BAC_SYNull() {
		pm_objDR["ID_BAC_SY"] = System.Convert.DBNull;
	}

	public string strHO_TEN 
	{
		get 
		{
			return CNull.RowNVLString(pm_objDR, "HO_TEN", IPConstants.c_DefaultString);
		}
		set 
		{
			pm_objDR["HO_TEN"] = value;
		}
	}

	public bool IsHO_TENNull() 
	{
		return pm_objDR.IsNull("HO_TEN");
	}

	public void SetHO_TENNull() {
		pm_objDR["HO_TEN"] = System.Convert.DBNull;
	}

	public decimal dcDOANH_THU 
	{
		get
		{
			return CNull.RowNVLDecimal(pm_objDR, "DOANH_THU", IPConstants.c_DefaultDecimal);
		}
		set	
		{
			pm_objDR["DOANH_THU"] = value;
		}
	}

	public bool IsDOANH_THUNull()	{
		return pm_objDR.IsNull("DOANH_THU");
	}

	public void SetDOANH_THUNull() {
		pm_objDR["DOANH_THU"] = System.Convert.DBNull;
	}

#endregion
#region "Init Functions"
    public void FillDatasetSearch(DS_V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0 ip_ds, string ip_str_search)
    {
        CStoredProc v_store_proc = new CStoredProc("pr_V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0_search");
        v_store_proc.addNVarcharInputParam("@STR_SEARCH", ip_str_search);
        v_store_proc.fillDataSetByCommand(this, ip_ds);
    }
	public US_V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0() 
	{
		pm_objDS = new DS_V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0();
		pm_strTableName = c_TableName;
		pm_objDR = pm_objDS.Tables[pm_strTableName].NewRow();
	}

	public US_V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0(DataRow i_objDR): this()
	{
		this.DataRow2Me(i_objDR);
	}

	public US_V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0(decimal i_dbID) 
	{
		pm_objDS = new DS_V_BAO_CAO_DOANH_THU_THEO_BAC_SY_0();
		pm_strTableName = c_TableName;
		IMakeSelectCmd v_objMkCmd = new CMakeAndSelectCmd(pm_objDS, c_TableName);
		v_objMkCmd.AddCondition("ID", i_dbID, eKieuDuLieu.KieuNumber, eKieuSoSanh.Bang);
		SqlCommand v_cmdSQL;
		v_cmdSQL = v_objMkCmd.getSelectCmd();
		this.FillDatasetByCommand(pm_objDS, v_cmdSQL);
		pm_objDR = getRowClone(pm_objDS.Tables[pm_strTableName].Rows[0]);
	}
#endregion
	}
}
